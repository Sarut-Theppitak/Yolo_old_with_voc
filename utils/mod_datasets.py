...
class BaseFunction:   # This class is the base fuction that every Datasets class should have
    ...
    def creat_label(self, bboxes):
        ...
        for bbox in bboxes: # start one by one gt box
            ...
            bbox_xywh_scaled = 1.0 * bbox_xywh[np.newaxis, :] / strides[:, np.newaxis]   # scale gt box to grid unit sp shape = [3,4] each row is at diferent scale
...
class YoloDataset(Dataset, BaseFunction):
    def __init__(self, cfg, train_dir, img_size=416, augmentation=True):
        ...
        anno_dir = os.path.join(train_dir,"labels")
    ...
    def __parse_data(self, img_path, anno):
        ...
        bboxes = anno[:, [1,2,3,4,0]]    # returb a copy so it will not change the original anno which we will use inthe next epoch
...
class VocDataset(Dataset, BaseFunction):
    def __init__(self, cfg, anno_file_type, img_size=416, augmentation=True):
        self.cfg_MODEL = cfg.MODEL
        self.cfg_PROJECT_PATH = cfg.PROJECT_PATH
        self.img_size = img_size  # For Multi-training
        self.classes = cfg.DATA["CLASSES"]
        self.num_classes = len(self.classes)
        self.class_to_id = dict(zip(self.classes, range(self.num_classes)))
        self.augmentation = augmentation
        self.__annotations = self.__load_annotations(anno_file_type) # all list in train.text
        # the boxese is in xyxy format 
    ...
    def __getitem__(self, item):
        ...
        img_org, bboxes_org = self.__parse_annotation(self.__annotations[item])   # bboxes_org in pixel xyxy
        img_org = img_org.transpose(2, 0, 1)  # HWC->CHW
    ...
    def __load_annotations(self, anno_type):
        ...
        anno_path = os.path.join(self.cfg_PROJECT_PATH, 'data', anno_type+"_annotation.txt")
...
if __name__ == "__main__":
    ...
    dataset = YoloDataset(cfg=cfg, train_dir=train_dir, img_size=cfg.TRAIN["TRAIN_IMG_SIZE"])
    dataloader = DataLoader(dataset, shuffle=True, batch_size=2, num_workers=0)
