...
class BaseEval():
    ...
    def __convert_pred(self, pred_bbox, test_input_size, org_img_shape, valid_scale):
        """
        预测框进行过滤，去除尺度不合理的框 The prediction box is filtered to remove the unreasonable scale
        """
...
class VocEvaluator(BaseEval):
    def __init__(self, model, cfg, mode, visiual=True):
        self.mode = mode
        if mode == 'test':
            mode_cfg = cfg.TEST
        elif mode == 'eval':
        ...
        self.predvoc_result_path = os.path.join(cfg.PROJECT_PATH, 'data', 'voceval_results')
        self.valvoc_data_path = cfg.VOC['VALVOC_DATA_PATH']
    ...
    def APs_run(self, multi_test=False, flip_test=False):
        img_inds_file = os.path.join(self.valvoc_data_path,  'ImageSets', 'Main', 'test.txt')
        with open(img_inds_file, 'r') as f:
            lines = f.readlines()
            img_inds = [line.strip() for line in lines]

        if os.path.exists(self.predvoc_result_path):
            shutil.rmtree(self.predvoc_result_path)  # delete the entire directory
        os.mkdir(self.predvoc_result_path)

        for img_ind in tqdm(img_inds):
            img_path = os.path.join(self.valvoc_data_path, 'JPEGImages', img_ind+'.jpg')
            img = cv2.imread(img_path)

            if img is None:
                print(f'****Could not read an image****: {img_path}')
            ...
            if bboxes_prd.shape[0]!=0 and self.__visiual and self.__visual_imgs < 100:
                ...
                path = os.path.join(self.predvoc_result_path, "{}.jpg".format(self.__visual_imgs))
            ...
            for bbox in bboxes_prd:
                ...
                with open(os.path.join(self.predvoc_result_path, 'comp4_det_test_' + class_name + '.txt'), 'a') as f:
                    f.write(s)
    ...
    def __calc_APs(self, iou_thresh=0.5, use_07_metric=False):
        ...
        :return:dict{cls:ap}
        """
        filename = os.path.join(self.predvoc_result_path, 'comp4_det_test_{}.txt')
        cachedir = os.path.join(self.predvoc_result_path, 'cache')
        annopath = os.path.join(self.valvoc_data_path, 'Annotations', '{}.xml')
        imagesetfile = os.path.join(self.valvoc_data_path,  'ImageSets', 'Main', 'test.txt')
...
class YoloEvaluator(BaseEval):
    def __init__(self, model, cfg, img_dir, mode, anno_dir=None):  # model are 'test' or 'eval'
        ...
        self.img_dir = img_dir    # img_dir
        self.anno_dir = anno_dir    # anno_dir
        self.__visual_imgs = 0
